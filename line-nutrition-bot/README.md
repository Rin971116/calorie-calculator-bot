...
在 LINE 傳一張食物照片，Bot 會辨識食物、確認不確定的項目、估算熱量與蛋白質，並存進 Notion。另外支援體重記錄、基礎代謝率估算、每日熱量盈餘計算。
...
| 🔥 計算基礎代謝率 | 點「計算基礎代謝率」→ 用過去一週體重變化 + 前一日攝取估算，更新代謝率表 |
| ➕ 熱量盈餘 | 點「熱量盈餘」→ 今日攝取 − 當前代謝率；缺資料會明確告知缺哪一項 |
...
## 基礎代謝率（BMR）估算邏輯
採「區間夾擠法」。把每日消耗近似為代謝率，並假設體重變化反映「前一天」攝取與消耗的落差：
- 某天**變輕** → 前一天消耗>攝取 → 代謝率 **大於** 前一日攝取（一個下界 x）
- 某天**變重** → 前一天消耗<攝取 → 代謝率 **小於** 前一日攝取（一個上界 y）

按下「計算基礎代謝率」時，統計過去 7 天：
`代謝率 ≈ ( max(所有下界 x) + min(所有上界 y) ) / 2`
...
- `計算基礎代謝率` → 一週估算並更新代謝率表
- `熱量盈餘` → 今日攝取減消耗（需先有今日餐點、今日體重、當前代謝率）

---

## 常見問題
**Q：熱量盈餘說缺資料？**
需同時具備今日餐點紀錄、今日體重、當前代謝率。缺哪項訊息會直接列出，補齊即可。
...
├── notion_service.py   # Notion：餐點/體重/統計/代謝率表
├── bmr_service.py      # 基礎代謝率區間夾擠 + 熱量盈餘
